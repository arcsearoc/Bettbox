...
| **PRELOAD_RACE_DIAL** | ❌ | `1`或`0` | **预加载竞速拨号**（本 fork 默认**开启**）；设`0`/`false`可关闭。解析 A/AAAA 后多 IP 竞速建连 |
| **TCP_CONCURRENT_DIAL**   | ❌ | `2` | **TCP 并发拨号数**，默认值为`2`；设置后不再根据中国移动网络自动降为单路 |
| **PROXY_CONCURRENT_DIAL** | ❌ | `2` | **反代并发拨号数**（本 fork 默认`2`）；数值越高连接速度越快，但 IP 切换也越频繁 |
| **CONNECT_TIMEOUT_MS** | ❌ | `1200` | **TCP 建连超时**（毫秒），默认`1200` |

> 更多 WS 优化说明见 [OPTIMIZATIONS.md](./OPTIMIZATIONS.md)
